...
- **Branch `development`** criada pelo usuário (2026-08-24) — o Vercel gera *preview deploy* automático (URL própria, não mexe em produção) pra qualquer push numa branch que não seja a `main`. Fluxo sugerido: commitar/testar na `development` (link de preview contra o LiveKit real), só dar merge pra `main` quando confiante — isso vai pro link que os amigos usam. Antes disso, testar mudança arriscada direto na `main` era o único jeito (ou não publicar).
